...
async function getDownloads(pkg: string) {
  const data = await (
    await fetch(`https://api.npmjs.org/downloads/point/last-week/${pkg}`)
  ).json();
  if (typeof data?.downloads !== "number") {
    throw new TypeError("Could not fetch npm downloads");
  }
...
export async function GET(req: Request) {
  ...
  const [inter900, inter700, inter400, stars, downloads] = await Promise.all([
    font("Inter", 900),
    font("Inter", 700),
    font("Inter", 400),
    getStars(props.repo),
    getDownloads(props.pkg),
  ]);
        ...
        <div tw="flex items-center text-neutral-300 my-8">
          <div tw="flex items-center mx-8">
            ...
            <div tw="flex items-center mx-8">
              ...
              <p tw="text-3xl font-bold ml-2">{downloads}</p>
            </div>
          </div>
        </div>
      </div>
